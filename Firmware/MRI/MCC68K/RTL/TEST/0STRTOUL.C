...
for (i=0; i<35; i++) {
    if ((tempLong = strtoul (&master[i], &retChar, i + 2)) != masterResult[i]) {
	errorFlag++;
	printf ("Error: strtoul 1:");
	printf (" i = %d input = \"%s\" expected = %d actual = %d\n",
	    i, &master[i], masterResult[i], tempLong);
	}
    if (retChar != &master[i+2]) {
	errorFlag++;
	printf ("Error: strtoul 2: bad address i = %d actual %p expected %p\n",
	    i, retChar, &master[i+2]);
	}
    }

for (i=0; i < (sizeof(strings) / sizeof(strings[0])); i++) {
    if ((tempLong = strtoul (strings[i].str, &retChar, strings[i].base)) != 
		strings[i].result) {
	errorFlag++;
	printf ("Error: strtoul 3: ");
	printf ("i = %d input = \"%s\" expected %d actual = %d\n",
