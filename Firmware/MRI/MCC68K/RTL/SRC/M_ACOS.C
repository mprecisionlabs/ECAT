...
#if EXCLUDE_m_acos || _80960SB || _80960KB || _80960MC || (_MCCG32 && _FPU) \
	|| (_MCC68K && _FPU) \
	|| (_MCC86 && (! _FPU)) \
	|| (__OPTION_VALUE ("-Kq"))
...
/*
 * @(SUBID) 05/10/88 /src2/nls/libraries/libm/s.acos.c 1.16
...
#define ACOS_EPS   (* (const double *) hex_acos_eps)
...
static const UInt32
	hex_acos_eps[2] =  HD_INIT(0x3E46A09E,0x667F3BCD), /* 2**(53/2) */
...
double	arg;
{
    ...
    if (isNAN(y)) {
	return( _fp_error((UInt32) (FP_acos | SIGNAN), y.value) );
    }
	...
	if (y.value > ONE)  		/* abs(arg) > 1 means error */
	    return( _fp_error((UInt32) (FP_acos | OPERROR)) );
 ...
 /* Step 8 */
	...
	if (y.value >= ACOS_EPS) {	/* significance? */
 /* Step 7 */	  
	    g.value = y.value * y.value;	/* calculate acos */
step9:          /* Step 9 */
	    y.value += y.value * (
    ...
    /* Now adjust result (if necessary) by PI/2 or PI depending upon
	       i, and sign of original argument */

    if (argsign) {
	return( (i) ? ((PI_OVER_4 + y.value) + PI_OVER_4) 
	            : ((PI_OVER_2 + y.value) + PI_OVER_2) );
    } 
    else {
	if (i)
	    return((PI_OVER_4 - y.value) + PI_OVER_4);
 /* Is it non-zero?  We do not want to return a negative zero. */
	if (y.value)
	    y.half.hi ^= 0x80000000;		/* Negate the result */
	return(y.value);
    }
}  /* acos() */

#endif /* EXCLUDE_m_acos */
