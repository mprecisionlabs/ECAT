...
#if EXCLUDE_m_logf || _80960SB || _80960KB || _80960MC || (_MCCG32 && _FPU) \
	|| (_MCC68K && _FPU) \
	|| (_MCC86 && (! _FPU)) \
	|| (!__OPTION_VALUE ("-Kq"))
...
double log(double arg)
{
    ...
    if (isNANf(f))
	return( _fp_errorf((UInt32) (FP_logf | SIGNAN), f.ivalue) );

    if ((n = isZEROf(f)) || isNEGf(f)) {
	f.value = _fp_errorf((UInt32) (FP_logf | OPERROR));
	if (n) {
	    f.ivalue = 0xFF800000;		/* Return neg. infinity */
	    errno = ERANGE;	/* Set to EDOM by fp_errorf(); correct it */
	}
...
#include "m_breakf.i"
	...
	(void) _fp_errorf((UInt32) (FP_logf | OPERROR));
	f.ivalue = 0xFF800000;		/* Return neg. infinity */
	return(f.value);
    }
 ...
 /* The number is normal.  Go ahead and compute the value. */
    ...
    f.ivalue |= (long)EXP_BIAS_MNS1_f << 23;
#endif
