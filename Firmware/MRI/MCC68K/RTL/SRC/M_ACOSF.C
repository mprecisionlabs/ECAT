...
#if EXCLUDE_m_acosf || _80960SB || _80960KB || _80960MC || (_MCCG32 && _FPU) \
	|| (_MCC68K && _FPU) \
	|| (_MCC86 && (! _FPU)) \
	|| (!__OPTION_VALUE ("-Kq"))
...
double acos(double arg) 
{
    ...
    if (isNANf(y))
	return( _fp_errorf((UInt32) (FP_acosf | SIGNAN), y.ivalue) );
    ...
    if (y.ivalue > HEX_HALF) {		/* compare with 1/2 */
   	if (y.ivalue > HEX_ONE)  	/* abs(arg) > 1 means error */
	    return( _fp_errorf((UInt32) (FP_acosf | OPERROR)) );
 ...
 /* Step 8 */
	...
	if (y.ivalue >= ACOS_EPS) {	/* significance? */
 /* Step 7 */
...
step9:          /* Step 9 */
 ...
 /* If result is non-zero negate it.  Do not want to return a neg. zero */
	if (y.ivalue)
	    y.ivalue ^= 0x80000000;
	return(y.value);
    }
