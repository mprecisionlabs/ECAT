...
;	op_Dmul.s
;
;	ST(1) * tos -> tos  (double precision multiply)
;	Readjust stack accordingly and return to the calling program.
;	Supplies function  __dmul()  that is used by the BE.
;	Calls fmuli
;
;	HISTORY:
;	5/25/89 mjf: Created
...
__dmul:
	...
	jsr	C_ADDR(fmuli)	; double mul routine (arg 2 on tos)
