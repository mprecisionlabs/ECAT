...
;	op_Dsub.s
;
;	ST(1) - tos -> tos  (double precision subtract)
;	Readjust stack accordingly and return to the calling program.
;	Supplies function  __dsub()  that is used by the BE.
;	Calls fsubi
;
;	HISTORY:
;	5/25/89 mjf: Created
...
__dsub:
	...
	jsr	C_ADDR(fsubi)	; double sub routine (arg 2 on tos)
				; deallocates 8 bytes (arg 2)
	move.l	(sp),-(sp)	; my return address -- making room
	move.l	d0,4(sp)
	move.l	d1,8(sp)
	rts
