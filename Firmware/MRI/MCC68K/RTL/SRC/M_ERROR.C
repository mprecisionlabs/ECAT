...
#if EXCLUDE_m_error || _80960SB || _80960KB || _80960MC \
	|| (__OPTION_VALUE ("-Kq"))
...
void	_err_handler();
int 	_denormalize(realnumber *, int, int);

double _fp_error(long caller_and_error, ...)
{
	STATIC va_list		arg_list;
	STATIC int		error_type;
	STATIC realnumber	mant;

	va_start(arg_list, caller_and_error);

	if ((error_type = (int) caller_and_error) & OPERROR) {
		errno = EDOM;
		mant.value = anNAN;		/* Return value is an NaN */
		error_type = OPERROR;
	}
		...
		mant.value   = va_arg(arg_list, double);
		round_sticky = va_arg(arg_list, int);

		error_type = _denormalize(&mant, round_sticky, shift_count);
	}
