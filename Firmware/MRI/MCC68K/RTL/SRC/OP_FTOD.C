...
;	op_FtoD.s
;
;	(float)tos -> (double) tos  (convert float to double)
;	Readjust stack accordingly and return to the calling program.
;	Supplies function  __ftod()  that is used by the BE.
;	Calls fdoublei
;
;	HISTORY:
;	5/25/89 mjf: Created
...
#pragma	asm
	...
	XDEF	__ftod
__ftod:
	move.l	4(SP),D0	; float
	jsr	C_ADDR(fdoublei);convert D0 (float) to D0:D1 (double)
