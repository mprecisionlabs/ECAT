...
;	op_Dcmp.s
;
;	ST(1) - tos -> cc  (double precision compare)
;	Readjust stack accordingly and return to the calling program.
;	Supplies function  __dcmp()  that is used by the BE.
;	Calls fcmpi
;
;	HISTORY:
;	5/25/89 mjf: Created
;	6/8/89  jpc: Removed pop of stuff which used to be left by "fcmpi"
...
#pragma	asm
	...
	XDEF	__dcmp
__dcmp:
	move.l	12(SP),D0	; arg 1, most significant 32 bits
	move.l	16(SP),D1	; least
	move.l	(SP)+,12(SP)	; save return address over arg 1t
	jsr	C_ADDR(fcmpi)	; double cmp routine
*
	movea.l	(SP)+,A0	; deallocate part of arg 1
	rts

#pragma	endasm
 
#endif /* EXCLUDE_op_dcmp */
