 ...
 *		pointed to by str to unsigned long integer.
 *
 *  05/21/85 - fixed bug for base 16 to allow "0x" or "0X"
 *  10/06/89 jpc: cast str to (char *)
 *  02/19/90 jpc: code move from strtol to strtoul
 *
 ************************************************************************/
...

{
	...
	{
	register unsigned long tempResult;	/* Temp for overflow check */

	tempResult = result;			/* Salt away result for a bit */
    	result = base*result + ch;
	if ((tempResult & 0xfc000000) && (result/base != tempResult)) {
						/* Overflow occurred */
