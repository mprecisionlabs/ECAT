...
;	op_Fadd.s
;
;	ST(1) + tos -> tos  (single precision add)
;	Readjust stack accordingly and return to the calling program.
;	Supplies function  __fadd()  that is used by the BE.
;	Calls faddi
;
;	HISTORY:
;	5/25/89 mjf: Created
...
__fadd:
	...
	move.l	(sp)+,(sp)	; move return addr. to correct location
*
	jsr	C_ADDR(fsaddi)	; D0 op D1 -> D0
*
