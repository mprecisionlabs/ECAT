...
;	op_LtoD.s
;
;	(long)D0 -> (double) tos  (convert long to double)
;	(long)(A0) -> (double) tos  (convert long to double)
;	Readjust stack accordingly and return to the calling program.
;	Supplies functions __dldl and __ltod()  used by the BE.
;	Calls fflti
;
;	HISTORY:
;	5/25/89 mjf: Created
...
__dldl:
	move.l	(A0),D0		; long
__ltod:
	jsr	C_ADDR(fflti)	; convert D0 (long) to D0:D1 (double)
	movea.l	(SP),A0		; my return address
	move.l	D1,(SP)		; low half
	move.l	D0,-(SP)	; high half
	jmp	(A0)

#pragma	endasm
 
#endif /* EXCLUDE_op_ltod */
