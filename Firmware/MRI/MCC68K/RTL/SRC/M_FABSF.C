...
#if EXCLUDE_m_fabsf || _80960SB || _80960KB || _80960MC || (_MCCG32 && _FPU) \
	|| (_MCC68K && _FPU) \
	|| (_MCC86 && (! _FPU)) \
	|| (!__OPTION_VALUE ("-Kq"))
...
#include "op_reent.h"
 ...
 /*--------------------------------------------------------------------
     ...
     This routine returns the absolute value of a float argument.
   If the argument is an NaN it is trasformed into a non-signaling NaN,
   but the sign bit is not altered.  For NaNs the sign bit may or may
   not be intended for use as the sign.		*/
...
double fabs(double arg)
{
    ...
    num.value = arg;
    if (isNANf(num))
	return( _fp_errorf((UInt32) (FP_fabsf | SIGNAN), num.ivalue) );
    else {
	num.ivalue &= 0x7FFFFFFF;	/* Clear the sign bit */
	return(num.value);
    }
}  /* fabsf() */
 
#endif /* EXCLUDE_m_fabsf */
