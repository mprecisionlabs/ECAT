...
;	Supplies function  __fcmp()  that is used by the BE.
;	Calls fscmpi
;
;	HISTORY:
;	5/25/89 mjf: Created
...
__fcmp:
	...
	movea.l	(SP)+,A0	; pop return address (cond codes untouched)
	addq.l	#8,SP		; deallocate both args (cond codes untouched)
	jmp	(A0)

#pragma	endasm
 
#endif /* EXCLUDE_op_fcmp */
