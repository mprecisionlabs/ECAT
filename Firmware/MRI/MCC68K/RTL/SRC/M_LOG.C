...
#if EXCLUDE_m_log || _80960SB || _80960KB || _80960MC || (_MCCG32 && _FPU) \
	|| (_MCC68K && _FPU) \
	|| (_MCC86 && (! _FPU)) \
	|| (__OPTION_VALUE ("-Kq"))
...
/*
 * @(SUBID) 05/10/88 /src2/nls/libraries/libm/s.log.c 1.15
...
double	arg; 
{
    ...
    if (isNAN(f)) {
	return( _fp_error((UInt32) (FP_log | SIGNAN), f.value) );
    }

    if ((n = isZERO(f)) || isNEG(f)) {
	f.value = _fp_error((UInt32) (FP_log | OPERROR));
	if (n) {
	    f.value = -HUGE_VAL;
...
#include  "m_break.i"
	...
	(void) _fp_error((UInt32) (FP_log | OPERROR));
	return(-HUGE_VAL);
    }
