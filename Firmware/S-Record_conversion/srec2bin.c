...
#define HEADER1 "\nSREC2BIN V1.20(thermo_build_1) - Convert Motorola S-Record to binary file.\n"
#define HEADER2 "Copyright (c) 2002 Ant Goffart - http://www.s-record.com/\n"
#define HEADER3 "Modifications by Frank Snyder Di Cesare, Thermo Fisher Scientific, Inc. 2007\n\n"
...
char filler = 0xff;
int verbose = TRUE;
int ignore_lower_addresses = FALSE;
   ...
   do
   {
      ...
      if (line[0] == 'S')                               /* an S-record */
      {
         ...
         if ((rec_type >= 1) && (rec_type <= 3))        /* data record */
         {
            ...
            if (scan)
            {
              if ( (*min > address) && (ignore_lower_addresses == FALSE) )
                  *min = address;
...
void process(void)
{
   ...
   if (verbose)
   {
      fprintf(stderr, "Minimum address  = %lXh\n", min_addr);
